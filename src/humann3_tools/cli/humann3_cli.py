...
def organize_output_files(results: Dict[str, Dict[str, str]], output_dir: str) -> Dict[str, str]:
    """
...
def parse_args():
    ...
    # Input options group (four methods supported)
    ...
    output_group.add_argument("--output-dir", default="./humann3_output",
                            help="Directory for HUMAnN3 output")
    output_group.add_argument("--organize-outputs", action="store_true", default=True,
                            help="Organize outputs into type-specific directories")
    ...
    humann3_group.add_argument("--humann3-options", nargs="+",
...
def main():
    ...
    # Process samples based on input method
    samples = {}
    
    # Method 1, 2, or 3: Use input handler
    if args.input_files or args.samples_file or (args.metadata_file and args.seq_dir):
    ...
    if args.use_parallel:
        ...
        for sample_id, sample_info in samples.items():
            ...
            if 'kneaddata_files' in sample_info and sample_info['kneaddata_files']:
                input_file = prepare_humann3_input(
            ...
            output_files = process_sample_humann3(
                sample_id=sample_id,
    ...
    logger.info("  humann3-tools join --input-dir PathwayAbundance --output-dir joined_output")
    logger.info("  humann3-tools join --input-dir GeneFamilies --output-dir joined_output")
...
Examples:
  ...
  humann3-tools humann3 --input-files sample1_cleaned.fastq --threads 8 --output-dir ./humann3_output
  ...
  humann3-tools humann3 --samples-file samples.txt --output-dir ./humann3_output
  
  # Using metadata:
  humann3-tools humann3 --metadata-file metadata.csv --seq-dir /path/to/cleaned_sequences --output-dir ./humann3_output
  
  # Specifying database paths:
  humann3-tools humann3 --input-files sample.fastq --nucleotide-db /path/to/chocophlan --protein-db /path/to/uniref --output-dir ./humann3_output
"""
...
try:
    from humann3_tools.utils.input_handler import get_input_files, find_kneaddata_output_files
...
def check_humann3_installation() -> Tuple[bool, str]:
...
def prepare_humann3_input(kneaddata_files: List[str], sample_id: str, output_dir: str, paired: bool = False) -> str:
    """
