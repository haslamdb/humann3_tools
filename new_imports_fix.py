...
1. Updates imports in Python files in the src directory:
   - Changes 'from humann3_tools.' to 'from src.humann3_tools.'
   - Changes 'import humann3_tools.' to 'import src.humann3_tools.'
...
def update_imports(file_path: str) -> Tuple[int, int]:
    ...
    for line in lines:
        # Skip lines that already have 'src.humann3_tools' to avoid double prefixing
        if 'src.humann3_tools' in line:
            modified_lines.append(line)
            continue
            
        # Match 'from humann3_tools.' pattern precisely
        from_pattern = r'^(\s*from\s+)(humann3_tools\.)(.+)$'
        ...
        # Match 'import humann3_tools.' pattern precisely
        import_pattern = r'^(\s*import\s+)(humann3_tools\.)(.+)$'
        ...
        # Match 'from humann3_tools import' pattern
        direct_import_pattern = r'^(\s*from\s+)(humann3_tools)(\s+import.+)$'
        ...
        # Match 'import humann3_tools' pattern
        simple_import_pattern = r'^(\s*import\s+)(humann3_tools)(\s|$)'
...
def update_entry_points(file_path: str) -> int:
    ...
    for line in lines:
        ...
        # Update entry points 
        if entry_point_section and '=' in line and 'humann3_tools.' in line and 'src.humann3_tools.' not in line:
            # Replace 'humann3_tools.' with 'src.humann3_tools.'
            new_line = line.replace('humann3_tools.', 'src.humann3_tools.')
            modified_lines.append(new_line)
            updates += 1
            continue
...
def update_script_files(script_dir: str) -> int:
    ...
    for file_name in script_files:
        ...
        # Look for imports of humann3_tools modules in executable scripts
        updated_content, count = re.subn(
            r'from humann3_tools\.', 
            'from src.humann3_tools.', 
            content
        )
        
        # Also update 'import humann3_tools' pattern
        further_updated, count2 = re.subn(
            r'import humann3_tools',
            'import src.humann3_tools',
            updated_content
        )
...
def test_imports() -> Dict[str, bool]:
    ...
    # Test 1: Import from src.humann3_tools package
    test_code = "import src.humann3_tools\nprint('Success')"
    ...
    test_code = "from src.humann3_tools.logger import setup_logger\nprint('Success')"
    with open(temp_file, 'w') as f:
        f.write(test_code)
...
def main():
    ...
    base_dir = "/home/david/Documents/Code/humann3_tools"
    ...
    print(f"Updated {total_from_count} 'from humann3_tools.' imports")
    print(f"Updated {total_import_count} 'import humann3_tools.' statements")
    print(f"Updated {entry_points_updated} entry points in pyproject.toml")
