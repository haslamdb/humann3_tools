...
Examples:
  ...
  python humann3_tools_preprocessing.py --input-fastq sample1_R1.fastq.gz sample1_R2.fastq.gz --paired --threads 8 --output-dir ./my_output
  
  # Running with multiple samples:
  python humann3_tools_preprocessing.py --input-fastq sample1_R1.fastq.gz sample1_R2.fastq.gz sample2_R1.fastq.gz sample2_R2.fastq.gz --paired --threads 8
  
  # Specifying database paths:
  python humann3_tools_preprocessing.py --input-fastq sample1.fastq.gz --kneaddata-dbs /path/to/kneaddata_db --humann3-nucleotide-db /path/to/chocophlan --humann3-protein-db /path/to/uniref
  
  # Skipping KneadData and using existing files:
  python humann3_tools_preprocessing.py --input-fastq sample1_R1.fastq.gz sample1_R2.fastq.gz --skip-kneaddata --kneaddata-output-files /path/to/kneaddata/sample1_paired_1.fastq /path/to/kneaddata/sample1_paired_2.fastq
...
# Set up logging
logger = logging.getLogger('humann3_preprocessing')
...
def prepare_humann3_input(kneaddata_outputs, paired=False):
    """
        ...
        if paired:
            ...
            try:
                ...
                if os.path.getsize(concat_file) > 0:
                    humann3_inputs[sample_id] = concat_file
            ...
            if files:
                ...
                if kneaddata_output_files:
                    ...
                    humann3_inputs[sample_id] = kneaddata_output_files[0]
                else:
                    # If no kneaddata-specific files, use the first available file
                    humann3_inputs[sample_id] = files[0]
                
                logger.info(f"Using file for HUMAnN3: {humann3_inputs[sample_id]}")
    
    return humann3_inputs
...
def run_humann3(input_files, output_dir, threads=1, nucleotide_db=None, protein_db=None, 
    ...
    for sample_id, input_file in input_files.items():
        ...
        humann3_results[sample_id] = sample_outputs
...
def print_sample_summary_table(results):
    ...
    """
    if not results or not results.get('humann3_results'):
        log_print("No results to display", level="warning")
        return
    
    # Calculate max width for sample names
    sample_names = list(results['humann3_results'].keys())
    ...
    for sample_id, files in results['humann3_results'].items():
...
def run_preprocessing_pipeline(input_files, output_dir, threads=1, 
                              ...
                              paired=False, kneaddata_options=None, humann3_options=None,
    ...
    humann3_input_files = prepare_humann3_input(kneaddata_results, paired=paired)
    ...
    logger.info(f"HUMAnN3 processing completed for {len(humann3_results)} samples")
...
def main():
    ...
    results = run_preprocessing_pipeline(
        ...
        humann3_options=humann3_options,
    ...
    log_print(f"HUMAnN3: Processed {len(results['humann3_results'])} samples", level="info")
    ...
    log_print(f"  HUMAnN3 Output: {args.humann3_output_dir or os.path.join(args.output_dir, 'humann3_output')}", level="info")
    
    # Print output files summary
    log_print("\nOutput Files Summary:", level="info")
    
    if results['humann3_results']:
        path_count = sum(1 for sample in results['humann3_results'].values() if sample.get('pathabundance'))
        gene_count = sum(1 for sample in results['humann3_results'].values() if sample.get('genefamilies'))
        pathcov_count = sum(1 for sample in results['humann3_results'].values() if sample.get('pathcoverage'))
        metaphlan_count = sum(1 for sample in results['humann3_results'].values() if sample.get('metaphlan'))
        
        if path_count > 0:
            pathabundance_dir = os.path.join(args.humann3_output_dir or os.path.join(args.output_dir, 'humann3_output'), "PathwayAbundance")
            log_print(f"  Pathway Abundance Files: {path_count} (in {pathabundance_dir})", level="info")
        
        if gene_count > 0:
            genefamilies_dir = os.path.join(args.humann3_output_dir or os.path.join(args.output_dir, 'humann3_output'), "GeneFamilies")
            log_print(f"  Gene Families Files: {gene_count} (in {genefamilies_dir})", level="info")
            
        if pathcov_count > 0:
            pathcoverage_dir = os.path.join(args.humann3_output_dir or os.path.join(args.output_dir, 'humann3_output'), "PathwayCoverage")
            log_print(f"  Pathway Coverage Files: {pathcov_count} (in {pathcoverage_dir})", level="info")
            
        if metaphlan_count > 0:
            metaphlan_dir = os.path.join(args.humann3_output_dir or os.path.join(args.output_dir, 'humann3_output'), "MetaphlanFiles")
    ...
    log_print("  To process these files with humann3_tools, you can use:", level="info")
    log_print("  humann3_tools --pathway-dir <pathabundance_dir> --gene-dir <genefamilies_dir> --sample-key <metadata.csv> --output-dir <output_dir>", level="info")
...
def parse_args():
    ...
    # HUMAnN3 options
    humann3_group = parser.add_argument_group("HUMAnN3 Options")
    humann3_group.add_argument("--humann3-nucleotide-db",
                             help="Path to HUMAnN3 nucleotide database (ChocoPhlAn)")
    humann3_group.add_argument("--humann3-protein-db",
                             help="Path to HUMAnN3 protein database (UniRef)")
    humann3_group.add_argument("--humann3-output-dir",
                             help="Directory for HUMAnN3 output files")
    humann3_group.add_argument("--bypass-prescreen", action="store_true",
                             help="Bypass the MetaPhlAn taxonomic prescreen")
    humann3_group.add_argument("--bypass-nucleotide-index", action="store_true",
                             help="Bypass the nucleotide index database")
    humann3_group.add_argument("--bypass-translated-search", action="store_true",
